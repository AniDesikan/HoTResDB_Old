...
## Note: To follow process, begin at "Main Code"

def log(output, header="", buffer=True, fname="/var/www/python_log.txt"):
	"""
...
def getGID(genelist, not_in_db):
	...
	for gene in genelist:
		cursor.execute("""
			SELECT GID 
			FROM GENES
			WHERE ensembl_human LIKE "{0}" OR ensembl_MMU LIKE "{0}" OR WikiGene_Name_human LIKE "{0}" OR WikiGene_Name_MMU LIKE "{0}";
		""".format(gene))
...
def createSampledict(samplelist, not_in_db):
	...
	for sample in samplelist:		
		...
		FROM EXPERIMENT
		WHERE STUDY_NAME LIKE "%s" AND EID in (SELECT DISTINCT EID
											   FROM PATIENT);
		"""%(sample))
		rows = cursor.fetchall()
...
"""
# TODO: 
# In the next three sections, the functions are all very similar.
# So similar that over the next iteration of revision, it is recommended that
# those sections are merged into one another.
# 
# This will reduce the codebase, though it will require some general 
# functions to be rewritten to pass more efficient messages back and forth.
"""
...
def count_boxplot_virus(geneDict, DPI_list, studies, noresults, s_status):
	...
	for key in geneDict:
		...
		for study in studies:
			...
			WHERE GID = %s AND Sequencing_type LIKE "%s" AND STUDY_NAME LIKE "%s" %s
			ORDER BY DPI;"""%(gid, "%" + seq_format + "%", "%" + study + "%", survival))
...
def countQuery_virus(geneDict, DPI_list, studies, noresults, s_status):
	...
	for key in geneDict:
		...
		for study in studies:
			...
			FROM GENES JOIN RNAseq using(gid) JOIN SAMPLE USING (sid) JOIN PATIENT USING (pid) JOIN EXPERIMENT USING (EID)
			WHERE gid = %s AND Study_Name LIKE "%s" %s
			GROUP BY DPI
			ORDER BY DPI;"""%(gid, "%" + study + "%", survival))
			...
			for key in geneDict:
				...
				for row in rows:
					...
					if current_exp != 0 and day_zero_exp != 0:
						FC = old_div(current_exp,day_zero_exp)
	...
	for gid in ProbeDict:
		...
		for study in studies:
			...
			sql = """SELECT DPI, avg(expression), virus, std(expression),count(expression), strain
					...
					WHERE probe_id IN (%s) AND STUDY_NAME LIKE "%s" %s
					GROUP BY DPI 
					ORDER BY DPI;"""
...
##############################
## IV. NANOSTRING FUNCTIONS ##
##############################

def nanostring_countQuery_virus (ProbeDict, geneDict, DPI_list, studies, noresults, s_status):
	"""
	# PURPOSE: Microarray counts data
	# INPUT
	# ProbeDict - DICTIONARY of probes, {gid: [probe_id1, probe_id2, probe_id3, ...]}
...
def nanostring_FCQuery_virus (probeDict, geneDict, DPI_list, studies, s_status, hm_x_axis):
	"""
	#PURPOSE: Nanostring fold change data
	#Note: refer to previous RNASeq pipeline for specific steps
	#Main difference: Need to average across probes for each MMU ID. Units are in normalized flourescence intensity 
	#initialize variables
	"""
...
nanostring_count_results = []
# if nanostring is selected
if "Nanostring" in datatypes:
	log("nano")
	nanostring_count_table = []
	nanostring_FC_results = []
	nanostring_FC_table = []
	NANOheat_map_sortedby_study = []

	nanostring_count_results, nanostring_count_table, noresults = nanostring_countQuery_virus(geneDict, strain, DPI_list, study_names, noresults, s_status)
	nanostring_FC_results, nanostring_FC_table, NANOheat_map_sortedby_study, heat_map_cats_sortedby_study = nanostring_FCQuery_virus(geneDict, strain, DPI_list, study_names, noresults, s_status, heat_map_sortedby_study, heat_map_cats_sortedby_study)
	
	#append tables and FC results but not count results
	count_table = count_table + nanostring_count_table
	FC_results = FC_results + nanostring_FC_results
	FC_table = FC_table + nanostring_FC_table
	heat_map_sortedby_study = heat_map_sortedby_study + NANOheat_map_sortedby_study
...
clust_heatmap = []
clust_heatmap_by_day = []

heat_map_sortedby_study_deep = copy.deepcopy(heat_map_sortedby_study)  # Remove any references to another object
heat_map_sortedby_study_deep2 = copy.deepcopy(heat_map_sortedby_study)

#Cluster Heatmaps results before sending to html
try:
	if len(geneDict) > 0: #check if any genes searched exist in database
		clust_heatmap_by_day = heatmap_cluster.main(heat_map_cats_sortedby_study[0], heat_map_sortedby_study_deep, "day")
		clust_heatmap = heatmap_cluster.main(heat_map_cats_sortedby_study[0], heat_map_sortedby_study_deep2, "")
except IndexError:
	pass


# TODO: Search tracking to find common searches
...
all_results.append(clust_heatmap)###################11
all_results.append(clust_heatmap_by_day)#			12
all_results.append(heat_map_sortedby_study)#########13
all_results.append(microarray_count_results)#		14
all_results.append(nanostring_count_results)########15
