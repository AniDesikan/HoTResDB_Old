...
###########
# Functions to enter RNA sequencing data
def write_to_csv(data):
    with open('/var/www/cgi-bin/rna_data.csv', 'a', newline='') as file:
...
if (form):
    ...
    # createSortableTables function in the html file
        ...
        FROM RNAseq_TEST r join SAMPLE_TEST st using(SID)
        where EID = %s;
        """
        ...
        from SAMPLE_TEST
        where SID not in (select SID from RNAseq_TEST rt) and EID = %s;
        """
    ...
    # Takes in all of the data in the form for the RNA_Sequencing data and inserts it into RNAseq_TEST
    ...
    # is keep this as an else statement and then 
        ...
        # This batch size is how many objects are entered into the sql database at once
        batch_size=1000
        # Reset the CSV file
        reset_csv_file()
        rna_data = []
        for idx, obj in enumerate(data):
                ...
                # Write the batch to CSV and clear the list
                
                if (idx + 1) % batch_size == 0:
                        write_to_csv(rna_data)
                        rna_data.clear()
